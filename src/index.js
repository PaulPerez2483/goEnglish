  ...
  const displayVideo = (video) => {
    ...
    if (right !== null) {
      right.addEventListener('click', () => {
           let section = document.getElementsByClassName('video-flex')[0];
        //    console.dir(section.children)
        if (count !== videos.length - 1) {
          // console.log('right')
          count = count + 1
          schoolTitle(video, count)
  
          let videosID = [...section.children].filter(el => el.tagName === 'SECTION');
